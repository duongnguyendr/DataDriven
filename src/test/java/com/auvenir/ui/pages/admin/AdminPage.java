...
//        validateDisPlayedElement(getEleAPIKeyTxt(), "API Text");
    ...
    public void verifyUserIsChangeStatusOnTheList(String email, String expectedStatus) {
        ...
        try {
            ...
            getLogger().info(e);
            NXGReports.addStep("Failed: Verify user is changed status on the list.", LogAs.FAILED, new CaptureScreen(CaptureScreen.ScreenshotOf.BROWSER_PAGE) ,e.getMessage());
        } catch (Exception e) {
            AbstractService.sStatusCnt++;
            getLogger().info(e);
            NXGReports.addStep("Failed: Verify user is changed status on the list.", LogAs.FAILED, new CaptureScreen(CaptureScreen.ScreenshotOf.BROWSER_PAGE), e.getMessage());
