...
public class CreateToDoTest extends AbstractTest {
	...
	public void verifyCreateToDoPageSearchData() throws Exception {
		try {
			auditorCreateToDoService.createToDoPage();
			auditorCreateToDoService.verifyDataSearch();
			NXGReports.addStep("Verify to create To-Do page and search data.", LogAs.PASSED, null);
		} catch (Exception e) {
			NXGReports.addStep("Verify to create To-Do page and search data.", LogAs.FAILED,
					new CaptureScreen(CaptureScreen.ScreenshotOf.BROWSER_PAGE));
			throw e;
		}
	}
